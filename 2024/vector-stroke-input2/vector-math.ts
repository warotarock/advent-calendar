...
export class VectorMath {
  ...
  /** 成分の値のセット */
  static set(dest: number[], x: number, y: number) {
    dest[0] = x;
    dest[1] = y;
  }

  /** 成分の値のコピー */
  static copy(dest: number[], src: number[]) {
    dest[0] = src[0];
    dest[1] = src[1];
  }
  ...
  static angleOfCorner(p1: number[], p2: number[], p3: number[]): number {
    const v1x = p1[0] - p2[0];
    const v1y = p1[1] - p2[1];
    const v2x = p3[0] - p2[0];
    const v2y = p3[1] - p2[1];
  ...
  static angleOfCurving(p1: number[], p2: number[], p3: number[]): number {
  ...
  static crossPoint(
    result: number[],
    l1p1: number[], l1p2: number[],
    l2p1: number[], l2p2: number[]
  ) {
  ...
  static distanceOfLineAndPoint(p1: number[], p2: number[], pt: number[]): number {
