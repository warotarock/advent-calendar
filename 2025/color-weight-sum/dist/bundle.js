...
(() => {
  ...
  function blendColors_StraightAlpha(colors, weights) {
    ...
    return new Color2(rSum, gSum, bSum, aSum);
  }
  function blendColors_PreMultipliedAlpha(colors, weights) {
    let wsum = 0, rSum = 0, gSum = 0, bSum = 0, aSum = 0;
    for (let i = 0; i < colors.length; i++) {
      const { r, g, b, a } = colors[i];
      const w = weights[i];
      wsum += w;
    ...
    return new Color2(rSum / (wsum * A), gSum / (wsum * A), bSum / (wsum * A), A);
  }
  function blendColors_OKLch(colors, weights) {
    let wsum = 0, Lsum = 0, Csum = 0, aSum = 0;
    const hueVec = { x: 0, y: 0 };
    for (let i = 0; i < colors.length; i++) {
      const { r: r2, g: g2, b: b2, a } = colors[i];
      const w = weights[i];
      const [L2, C2, h2] = chroma_js_default.rgb(r2 * 255, g2 * 255, b2 * 255).oklch();
      wsum += w;
      Lsum += w * L2 * a;
      Csum += w * C2 * a;
      aSum += w * a;
      if (!isNaN(h2)) {
        hueVec.x += w * C2 * Math.cos(h2 / 180 * Math.PI) * a;
        hueVec.y += w * C2 * Math.sin(h2 / 180 * Math.PI) * a;
      }
    }
    if (wsum === 0) {
      return new Color2(0, 0, 0, 0);
    ...
    const [r, g, b] = chroma_js_default.oklch(L, C, h * 180 / Math.PI).rgb();
    return new Color2(r / 255, g / 255, b / 255, A);
  }
  function getColorElement(parent, i, type) {
    return parent.querySelector(`.${type}${i}`);
  }
  function addColorSettingRow(controls, className, color, weight) {
  ...
  function getColorsAndWeights(controls) {
    ...
    rows.forEach((_row, i) => {
      ...
      const rgb3 = chroma_js_default(color).rgb();
      colors.push(new Color2(rgb3[0] / 255, rgb3[1] / 255, rgb3[2] / 255, alpha));
      weights.push(weight);
    });
    return { colors, weights };
  }
  function updateCanvas(blendedColor, colors, weights, ctx, canvas) {
    if (!ctx) {
      return;
    }
  ...
  function initializeMethodContainer(containerID, method) {
    const methodContainer = document.getElementById(containerID);
    const controls = methodContainer.querySelector(".controls");
    const canvas = methodContainer.querySelector(".preview-canvas");
    const ctx = canvas.getContext("2d");
    const initialColors = [
      { color: "#ff8800", weight: 1 },
    ...
    methodContainer.querySelector(".addColor").addEventListener("click", () => {
      addColorSettingRow(controls, colorCount, "#ffffff", 1);
      colorCount++;
      redrawCanvas();
    });
    methodContainer.querySelector(".removeColor").addEventListener("click", () => {
